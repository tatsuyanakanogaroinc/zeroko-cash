-- Row Level Security (RLS) ポリシーの設定 (修正版)
...
DROP POLICY IF EXISTS "Admins and approvers can view all expenses" ON public.expenses;
DROP POLICY IF EXISTS "Admins and approvers can update expense status" ON public.expenses;

DROP POLICY IF EXISTS "Users can view own invoice payments" ON public.invoice_payments;
DROP POLICY IF EXISTS "Users can create own invoice payments" ON public.invoice_payments;
DROP POLICY IF EXISTS "Users can update own pending invoice payments" ON public.invoice_payments;
DROP POLICY IF EXISTS "Admins and approvers can view all invoice payments" ON public.invoice_payments;
DROP POLICY IF EXISTS "Admins and approvers can update invoice payment status" ON public.invoice_payments;
...
DROP POLICY IF EXISTS "All users can view approvers" ON public.approvers;
DROP POLICY IF EXISTS "Admins can manage approvers" ON public.approvers;
...
BEGIN
    ...
    RAISE NOTICE 'UUID type casting has been properly applied.';
    RAISE NOTICE '=======================================';
END $$;
